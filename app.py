...
UI_TEXT = {
    ...
    "Spanish": {
        ...
        "escape_doc_section": "📄 Veamos (leamos) con qué necesitas ayuda.",
    ...
    "Portuguese": {
        ...
        "privacy_notice_box": '<div style="font-size: 0.8rem; padding: 10px; border-radius: 6px; background-color: rgba(2, 132, 199, 0.08); border-left: 4px solid #0284C7; margin-bottom: 15px;">🔒 <strong>Aviso de privacidade:</strong> Não guardamos nada.</div>',
    ...
    "Japanese": {
        ...
        "start_over": "🧹 最初からやり直す",
        "terms_button": "📜 利用規約",
        "privacy_notice_box": '<div style="font-size: 0.8rem; padding: 10px; border-radius: 6px; background-color: rgba(2, 132, 199, 0.08); border-left: 4px solid #0284C7; margin-bottom: 15px;">🔒 <strong>プライバシー通知:</strong> データの保存は一切行いません。提供されたドキュメントや機密情報は一時的に処理され、保存されることはありません。安全かつプライベートに分かりやすく解説します。</div>',
    ...
    "Mandarin": {
        ...
        "start_over": "🧹 重新开始",
        "terms_button": "📜 条款与条件",
        "privacy_notice_box": '<div style="font-size: 0.8rem; padding: 10px; border-radius: 6px; background-color: rgba(2, 132, 199, 0.08); border-left: 4px solid #0284C7; margin-bottom: 15px;">🔒 <strong>隐私声明：</strong> 我们不保存任何内容。您提供的任何文档或敏感信息均经过临时处理，绝不存储。我们以安全、私密的方式让您轻松理解。</div>',
    ...
    "Hindi": {
        ...
        "privacy_notice_box": '<div style="font-size: 0.8rem; padding: 10px; border-radius: 6px; background-color: rgba(2, 132, 199, 0.08); border-left: 4px solid #0284C7; margin-bottom: 15px;">🔒 <strong>गोपनीयता सूचना:</strong> हम कुछ भी सहेजते नहीं हैं। आपके द्वारा प्रदान किए गए दस्तावेज़ क्षणिक रूप से संसाधित किए जाते हैं।</div>',
    ...
    "Korean": {
        ...
        "start_over": "🧹 처음부터 다시",
        "terms_button": "📜 이용약관",
        "privacy_notice_box": '<div style="font-size: 0.8rem; padding: 10px; border-radius: 6px; background-color: rgba(2, 132, 199, 0.08); border-left: 4px solid #0284C7; margin-bottom: 15px;">🔒 <strong>개인정보 보호 안내:</strong> 저장되는 것은 아무것도 없습니다. 제공해주신 문서나 민감한 정보는 일시적으로 처리되며 절대 저장되지 않습니다. 안전하고 프라이빗하게 이해하기 쉽게 설명해 드립니다.</div>',
...
if is_streamlit:
    ...
    # Global Client Initialization (Vertex AI)
    client = genai.Client(
        vertexai=True,
...
# ==============================================================================
# [SECTION 7: MAIN TAB NAVIGATION SETUP]
# ==============================================================================
if is_streamlit:
            ...
            if submitted:
                if not topic and audio_value is None:
                    ...
                    with st.spinner(spinner_text):
                        try:
                            full_prompt = f"Explain the following topic with a tone of {tone_level} and a depth level of {depth_level}: {topic}"
...
with tab3:
    ...
    def clear_escape_data():
        ...
        if "escape_extra_hint_input_unique" in st.session_state:
            st.session_state["escape_extra_hint_input_unique"] = ""
    ...
    if run_escape_decode or st.session_state.get("trigger_end_suffering_exec", False):
        ...
        api_key = os.getenv("GEMINI_API_KEY", "")
        if not api_key:
            st.error(texts.get("no_api", "API key not found."))
