...
# ==============================================================================
# [SECTION: TAB 4 - GLOBAL AI RESOURCE & INFRASTRUCTURE DASHBOARD]
# ==============================================================================
with tab4:
    st.markdown('<div class="app-title">⏱️ Global AI Infrastructure Intelligence</div>', unsafe_allow_html=True)
    st.markdown('<div class="app-subtitle">Real-Time Resource Footprint, Energy Demand, & Water Metrics</div>', unsafe_allow_html=True)
    st.markdown("---")

    # --- NEW: COMPLEXITY TIER SELECTOR FOR TAB 4 ---
    dash_tier = st.radio(
        "Select Dashboard Complexity Tier:",
        ["🟢 Easy (Everyday Reality)", "🟡 Balanced (Infrastructure Bottleneck)", "🔴 Hard (Grid Physics & Thermodynamics)"],
        horizontal=True,
        key="tab4_complexity_tier"
    )
    st.markdown("---")

    # Top-Level Summary Metrics (Global Scale - Always Visible)
    ...
    if "Easy" in dash_tier:
        st.markdown("### 🟢 What Does All This Mean? (The Everyday Reality)")
        st.info(
            "**The Simple Truth:** AI isn't floating in a magical cloud—it lives inside massive warehouses filled with millions of hot computers. "
            "Every time you ask an AI a question, chips work hard and generate heat. To keep them from melting, these warehouses suck up millions of gallons of water "
            "for cooling and pull massive amounts of electricity right off the local power grid. AI has a physical, heavy footprint just like a factory."
        )
    elif "Balanced" in dash_tier:
        st.markdown("### 🟡 What Does All This Mean? (The Infrastructure Bottleneck)")
        st.warning(
            "**The Operational Challenge:** We are running into a physical wall of engineering. Tech companies want to build bigger AI models, "
            "but **the power grid can't keep up**. Traditional power plants take years to build, while AI clusters scale in months. "
            "This is why tech giants are scrambling to secure nuclear power, build massive solar farms, and invent advanced liquid cooling systems. "
            "The limiting factor for tech isn't code anymore—it's electricity and water."
        )
    else:
        st.markdown("### 🔴 What Does All This Mean? (Grid Physics & Thermodynamics)")
        st.error(
            "**The Deep Technical Reality:** Hyperscale cluster density is pushing PUE (Power Usage Effectiveness) and grid tolerances to their limits. "
            "When individual campuses demand 1 Gigawatt of continuous power—equal to a medium city—they destabilize regional transmission lines. "
            "This forces a structural shift toward **off-grid co-location**: pairing data centers directly with dedicated nuclear SMRs or localized solar-battery micro-grids "
            "to bypass public grid collapse entirely."
        )

    st.markdown("---")

    # --- PLOTLY GAUGES & CALCULATOR REMAIN BELOW ---
    # (Keep your existing Plotly code and Interactive Calculator code right here...)
